...
(function() {
    ...
    window.EnhancedScanner = {
        ...
        findAndEnhanceForms: function() {
            const forms = document.querySelectorAll('form[data-scan-form="true"]:not([data-scanner-enhanced="true"]), #footer-scanner-form:not([data-scanner-enhanced="true"]), #enhanced-scanner-form:not([data-scanner-enhanced="true"])');
            let enhanced = 0;
            
            forms.forEach(form => {
                const urlField = form.querySelector('input[data-url-field="true"], input[id*="website-URL"], input[name*="website-URL"], input[type="url"]');
        ...
        enableSubmitButtons: function() {
            document.querySelectorAll('form input[type="submit"][disabled], form button[type="submit"][disabled]').forEach(button => {
                ...
                if (form && form.querySelector('input[data-url-field="true"], input[id*="website"], input[name*="website"]')) {
        ...
        handleContactSubmit: function(e) {
            ...
            if (!phone || !email) {
                alert("Både telefonnummer och e-postadress krävs");
                return false;
            }
            
            if (!consent) {
                alert("Vänligen acceptera villkoren för att fortsätta");
                return false;
            }
            
            this.session.contactInfo = {
                name: "Ej angivet",
                email: email,
                phone: phone
            };
            ...
            console.log("Contact info collected from modal:", this.session.contactInfo);
            
            this.clearWebhookTimers();
            
            if (window.ScannerUI && typeof window.ScannerUI.hideContactModal === 'function') {
            ...
            if (modal) {
                ...
                console.log("Showing pre-loaded report modal");
            }
        ...
        analyzeReportContent: function(content) {
            ...
            const patterns = [
                ...
                /(?:total|number of)\s*(?:issues?|problems?|errors?|violations?):\s*(\d+)/i
            ];
        ...
        showReportModal: function() {
            if (window.ScannerUI && typeof window.ScannerUI.showReportModal === 'function') {
                const reportUrl = this.session.brandedReportUrl || this.session.whitelabelReportUrl || this.session.reportUrl;
                window.ScannerUI.showReportModal(reportUrl, this.config.enableCTAOverlay, this.session);
            }
        },
        
        resolveModalConflicts: function() {
            const oldModal = document.getElementById("modal66f5d0180130eb9ebefb1233");
            const newModal = document.getElementById("enhancedModal");
            
            if (!oldModal || !newModal) return false;
            
            if (oldModal.style.display === "block" && newModal.style.display === "block") {
