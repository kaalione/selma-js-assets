/**
 * Enhanced Scanner Core v3.6
 * Core scanning functionality with deep analysis and minimizable CTA
 * Updated: 2025-09-30 (Version sync with ScannerUI v3.6)
 */
(function () {
    'use strict';
    ...
    window.EnhancedScanner = {
        ...
        init: function () {
            // UPDATED: Version to v3.6
            console.log("Enhanced Scanner Core v3.7 initializing...");
            ...
            try {
                ...
                if (window.ScannerUI && typeof window.ScannerUI.init === 'function') {
                    ...
                    console.log("ScannerUI not yet loaded, will initialize when available");
                }
                ...
                if (window.ScannerTracking && typeof window.ScannerTracking.init === 'function') {
                    ...
                    console.log("ScannerTracking not yet loaded, will initialize when available");
                }
                ...
                if (window.ScannerResources && typeof window.ScannerResources.init === 'function') {
                    ...
                    console.log("ScannerResources not yet loaded, will initialize when available");
                }
        ...
        trackPartnerActivity: function () {
            ...
            window.dataLayer.push({
                ...
                // UPDATED: Version tag to v3.6
                scannerVersion: "enhanced_v3.6_minimizable"
            });
        },
        ...
        enhanceForm: function (form, urlField) {
            ...
            // UPDATED: Version tag to v3.6
            form.setAttribute("data-scanner-version", "enhanced_v3.6_minimizable");
        ...
        handleReportContactFormSubmit: function (e, form) {
            e.preventDefault();
            e.stopPropagation();
            ...
            this.session.contactInfo.email = emailField ? emailField.value.trim() : null;
            this.session.contactInfo.phone = phoneField ? phoneField.value.trim() : null;

            // Send the final webhook
            this.sendToAccessiBe();
            this.clearWebhookTimers();
            this.sendWebhook("scanner_lead_complete");


            console.log("Contact form submitted - proceeding to report");

            // Hide the contact form
            window.ScannerUI.hideContactModal();
        },
        ...
        startScan: function (domain, formType = "simple", contactInfo = null) {
            ...
            this.session.initialWebhookSent = false;
            this.session.complianceStatus = null;
            this.session.reportResult = null;
            ...
            // UPDATED: Version tag to v3.6
            this.trackEvent("enhanced_scanner_submit_v3_6", {
                scanned_domain: cleanedDomain,
                form_type: formType,
                has_contact_info: !!(contactInfo && contactInfo.email),
                scanner_version: "enhanced_v3.6_minimizable",
            ...
            if (formType === "contact" && contactInfo && contactInfo.email) {
                // EDVIN: I dont even know when this would not be true. Ignoring for minimal changes and potential side effects
                ...
                // Send Webhook to capture lead withohut contact info
                this.sendWebhook("scanner_lead_initiated");
                this.startBackgroundReportLoading();
                
                // Show early contact modal before data is loaded
                setTimeout(() => {
                    if (window.ScannerUI && typeof window.ScannerUI.showEarlyContactModal === 'function') {
                        window.ScannerUI.showEarlyContactModal(this.session);
                    }
        ...
        startBackgroundReportLoading: function () {
            // Verify ScannerUI is available AND initialized
            if (!window.ScannerUI ||
                !window._scannerUIInitialized ||
                typeof window.ScannerUI.startBackgroundReportLoading !== 'function') {
            ...
            window.ScannerUI.startBackgroundReportLoading(this.session.reportUrl, () => {
                ...
                console.log("Report modal loaded successfully");

                // Show CTA overlay if enabled
                if (this.config.enableCTAOverlay &&
                    window.ScannerUI.showCTAOverlay &&
                    typeof window.ScannerUI.showCTAOverlay === 'function') {
                    setTimeout(() => {
        ...
        sendWebhook: function (action = "scanner_lead_complete") {
            ...
            if (action === "scanner_lead_complete") {
                if (this.session.finalWebhookSent === true) {
            ...
            if (action === "scanner_lead_initiated") {
                if (this.session.initialWebhookSent === true) {
                    console.log("Initial webhook already sent, skipping");
                    return Promise.resolve("already_sent");
                }
                this.session.initialWebhookSent = true;
            }
            ...
            const webhookData = {
                action: action,
                name: this.session.contactInfo.name || "",
                email: this.session.contactInfo.email || "",
                phone: this.session.contactInfo.phone || "",
            ...
            if (window.selmaDebugLocal || webhookData.website.includes("selma.se")) {
                console.log("Debug mode - webhook not sent:", webhookData);
                return false;
            }
        ...
        setupMessageListeners: function () {
            ...
            window.addEventListener("message", function (event) {
                if (event.data && event.data.acsbAceResults) {
                    console.log("AccessiBe results received, processing...", event.data);
        ...
        processAccessiBeResults: function (data) {
            try {
                ...
                this.session.reportResult = data;
                ...
                // Calculate total failures count across all reports
                let totalFailures = 0;
                if (data.reports) {
                    Object.values(data.reports).forEach(category => {
                        if (category && typeof category === 'object') {
                            Object.values(category).forEach(subcategory => {
                                if (subcategory && typeof subcategory.failures === 'number') {
                                    totalFailures += subcategory.failures;
                                }
                            });
                        }
                    });
                }
                this.session.issuesCount = totalFailures;
                console.log("Total issues counted:", totalFailures);
                console.log("Compliance status determined:", this.session.complianceStatus);

                /*
                // EDVIN: Old parsing logic - kept for reference
                // No need for complex parsing when structured data is available
                ...
                */
                ...
                // Old contact modal logic - kept for reference, replaced with showEarlyContactModal
                ...
                //
                //     console.log("Scheduling contact modal with compliance info in (ms):", this.config.reportAnalysisDelay ?? 0);
                //     setTimeout(() => {
                //         window.ScannerUI.showContactModal(false, this.session)
                //
                //         // Bind submission handler to contact form
                //         const contactForm = document.getElementById("contactForm");
                //         if (contactForm) {
                //             contactForm.addEventListener("submit", (e) => this.handleReportContactFormSubmit(e, contactForm), true);
                //         } else {
                //             throw new Error("Contact form not found for enhancement");
                //         }
                //
                //     }, this.config.reportAnalysisDelay ?? 0);
                // }
        ...
        addPartnerDataToForm: function (form) {
            ...
            const fieldsToAdd = [
                ...
                // UPDATED: Version tag to v3.6
                { name: "scanner_version", value: "enhanced_v3.6_minimizable" },
        ...
        onResourceLoaded: function (resourceName) {
            ...
            // Re-attempt initialization if core modules are now available
            if (resourceName === 'ScannerUI' && window.ScannerUI && !window._scannerUIInitialized) {
                try {
                    window.ScannerUI.init();
                    console.log("ScannerUI initialized after resource load");
                } catch (e) {
                    console.warn("Delayed ScannerUI init failed:", e);
                }
            }

            if (resourceName === 'ScannerTracking' && window.ScannerTracking && typeof window.ScannerTracking.init === 'function') {
                try {
                    window.ScannerTracking.init();
                    console.log("ScannerTracking initialized after resource load");
                } catch (e) {
                    console.warn("Delayed ScannerTracking init failed:", e);
                }
            }

            if (resourceName === 'ScannerResources' && window.ScannerResources && typeof window.ScannerResources.init === 'function') {
                try {
                    window.ScannerResources.init();
                    console.log("ScannerResources initialized after resource load");
                } catch (e) {
                    console.warn("Delayed ScannerResources init failed:", e);
                }
            }
        }
    };
