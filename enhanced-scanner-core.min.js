...
(function() {
    ...
    // Show the report in iframe for all form types
    if (window.ScannerUI && typeof window.ScannerUI.showReportModal === 'function') {
        window.ScannerUI.showReportModal(this.session.reportUrl);
        
        // Set up background report loading with CTA
        if (typeof window.ScannerUI.startBackgroundReportLoading === 'function') {
            window.ScannerUI.startBackgroundReportLoading(this.session.reportUrl, () => {
                // Report loaded callback
        ...
        // Fallback to new tab if UI not available
